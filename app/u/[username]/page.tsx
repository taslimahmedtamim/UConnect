...
import { 
  MapPin, Briefcase, Award, ShieldCheck, Link as LinkIcon, GraduationCap, Users, FolderOpen, ExternalLink, Code
} from "lucide-react";
    ...
    <div className="max-w-5xl mx-auto py-12 px-4 sm:px-6">
      ...
      <div className="space-y-6">
            ...
            <div className="flex flex-wrap gap-3">
              {user.skills.map((skill: any, idx: number) => {
                ...
                return (
                  ...
                  >
                    <span className="text-blue-600 dark:text-blue-400 font-medium">{skillName}</span>
                  </div>
                );
              })}
            </div>
        ...
        {user.githubUsername && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
              <Code className="w-5 h-5 text-slate-700 dark:text-slate-300" /> GitHub Open Source Stats
            </h2>
        ...
        {user.projects && user.projects.length > 0 && (
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-8">
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
              <FolderOpen className="w-5 h-5 text-indigo-500" /> Projects Portfolio
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {user.projects.map((project: any) => (
                <div key={project.id} className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 border border-slate-100 dark:border-slate-800 flex flex-col h-full hover:border-indigo-200 dark:hover:border-indigo-700 transition-colors">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {(Array.isArray(project.tags) ? project.tags : []).slice(0, 3).map((tag: string) => (
                      <span key={tag} className="px-2 py-0.5 rounded-md text-[10px] font-medium bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-300">
                        {tag}
                      </span>
                    ))}
                    {(Array.isArray(project.tags) ? project.tags.length : 0) > 3 && (
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-medium bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400">
                  ...
                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-200 dark:border-slate-700/50">
                    ...
                    <button 
                      ...
                      className="text-xs font-bold bg-indigo-50 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 px-3 py-1.5 rounded-lg hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors flex items-center gap-1.5"
                    >
                      View Details
                    </button>
                  </div>
